...
OUT=$(aws sts assume-role --role-arn $ROLEARN --role-session-name $ROLESESSION);sleep 15s;\
...
if [[ $? != 0 ]]; then
	print_color 'red' 'Configuration was unsucessful, some issue with role-arn/session/permission'
	quit_execution
fi
